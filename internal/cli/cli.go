...
// The question is printed to the writer, and the answer is read from the reader.
func StringQuestion(writer io.Writer, reader *bufio.Reader, label string, defaultValue string, validations ...ValidationFunc[string]) (string, error) {
	if defaultValue == "" {
...
// UntypedQuestion asks a question that expects an untyped answer.
func UntypedQuestion(writer io.Writer, reader *bufio.Reader, label string, defaultValue any, validations ...ValidationFunc[any]) (any, error) {
	if defaultValue == nil {
		fmt.Fprintf(writer, "%s: ", label)
	} else {
		fmt.Fprintf(writer, "%s [%s]: ", label, defaultValue)
